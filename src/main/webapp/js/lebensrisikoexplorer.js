	...
	d3.json("rest/waypoints",
		...
		function barChart() {
		    ...
		    return d3.rebind(chart, brush, "on");
				...
				// Create the crossfilter for the relevant dimensions and
				// groups.
				var waypoint = crossfilter(waypoints), all = waypoint
						.groupAll(), date = waypoint.dimension(function(d) {
					return new Date(d[1]);
				}), dates = date.group(d3.time.day);

				var charts = [ barChart().dimension(date).group(dates).round(
						d3.time.day.round).x(
						d3.time.scale()
								.domain(
										[ new Date(2015, 0, 1),
												new Date(2015, 11, 31) ])
								.rangeRound([ 0, 10 * 90 ])).filter(
						[ new Date(2015, 0, 1), new Date(2015, 11, 31) ]) ];

				// Given our array of charts, which we assume are in the same
				// order as the
				// .chart elements in the DOM, bind the charts to the DOM and
				// render them.
				// We also listen to the chart's brush events to update the
				// display.
